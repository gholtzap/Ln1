...
final class Ln1SmokeTests: Ln1TestCase {
    func testPolicyCommandReturnsKnownActionRiskClassifications() throws {
        ...
        XCTAssertEqual(actionByName["workflow.status"]?["domain"] as? String, "workflow")
        XCTAssertEqual(actionByName["workflow.status"]?["risk"] as? String, "medium")
        XCTAssertEqual(actionByName["workflow.status"]?["mutates"] as? Bool, false)
