...
- Browser action preflight: `03 workflow preflight --operation fill-browser|select-browser|click-browser|navigate-browser ...` validates DevTools, audit-log readiness, tab IDs, selectors, text, select option values/labels, URLs, and match modes before returning typed browser action argv arrays.
