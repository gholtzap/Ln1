...
- DevTools DOM state attributes: `03 browser dom` includes selected ARIA state attributes such as `aria-expanded`, `aria-selected`, `aria-pressed`, and `aria-controls` so agents can discover UI state before choosing a wait or action.
